...
public final class ResizableArrayBag<T> implements BagInterface<T>
{
   ...
   /**Takes the contents of two bags and merges it into one bag
      @param bag object
      @return bag with the combined contents of two bags */
   public BagInterface<T> union(BagInterface<T> anotherBag)
   {
         checkintegrity();
   
         @SuppressWarnings("unchecked")
         ResizableArrayBag<T> everythingBag = new ResizableArrayBag();                                                           
   
         T[] tempBag = anotherBag.toArray(); 
         
         for(int i = 0; i < this.getCurrentSize(); i++){
               everythingBag.add(bag[i]);                                                
         }//end for
   
         for(int i = 0; i < anotherBag.getCurrentSize(); i++){
               everythingBag.add(tempBag[i]); 
         }//end for
   
   
         return everythingBag;
   } //end union
